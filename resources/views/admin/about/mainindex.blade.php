...
<style>
    ...
    /* CKEditor இன் உயரத்தை மற்றும் எழுத்து நிறத்தை சரிசெய்ய */
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        // CKEditor 4 ஐ எளிய முறையில் லோடு செய்ய
        if (document.querySelector('#input-desc')) {
            CKEDITOR.replace('input-desc', {
                versionCheck: false, // பதிப்பு எச்சரிக்கைகளைத் தவிர்க்க
                height: 250,
                removeButtons: 'About,Subscript,Superscript,Scayt'
            });
            console.log("CKEditor 4 வெற்றிகரமாக லோடு ஆகிவிட்டது!");
        }

        // Form Submit Code (Vanilla JS - No Conflict)
        const form = document.getElementById('hero-form');
        if (form) {
            form.addEventListener('submit', function(e) {
                e.preventDefault();
                
                // CKEditor 4 இன் தரவை textarea-விற்கு மாற்றுதல்
                if (CKEDITOR.instances['input-desc']) {
                    document.getElementById('input-desc').value = CKEDITOR.instances['input-desc'].getData();
                }
                ...
                .then(response => {
                    ...
                    submitBtn.innerHTML = '<i class="fa-solid fa-floppy-disk me-2"></i> Save & Apply Changes';
                    Swal.fire({ icon: 'success', title: 'Success!', text: 'Changes saved successfully!', showConfirmButton: false, timer: 2000 });
                })
                .catch(() => {
                    submitBtn.disabled = false;
                    submitBtn.innerHTML = '<i class="fa-solid fa-floppy-disk me-2"></i> Save & Apply Changes';
